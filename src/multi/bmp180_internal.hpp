...
namespace hal::sensor::bmp180_internal {
    ...
    // datasheet.
    inline computation_variables calculate_true_temperature(std::int32_t p_uncompensated_temperature, std::uint16_t p_ac5, std::uint16_t p_ac6, std::int16_t p_mc, std::int16_t p_md) {
    ...
    // following the exact algorithm for conversion given in figure 4 of the
    // datasheet.
    inline computation_variables calculate_true_pressure(std::int32_t p_uncompensated_pressure, std::int16_t p_oversampling_setting, std::int32_t p_b5, std::int16_t p_ac1, std::int16_t p_ac2, std::int16_t p_ac3, std::uint16_t p_ac4, std::int16_t p_b1, std::int16_t p_b2) {
        computation_variables variables;
        
        variables.b6 = p_b5 - 4000;
        variables.x1 =
            (p_b2 * ((variables.b6 * variables.b6) >> 12)) >> 11;
        variables.x2 = (p_ac2 * variables.b6) >> 11;
        variables.x3 = variables.x1 + variables.x2;
        variables.b3 =
            ((((static_cast<std::int32_t>(p_ac1)) * 4 + variables.x3)
            << p_oversampling_setting) + 2) >> 2;
        variables.x1 = (p_ac3 * variables.b6) >> 13;
        variables.x2 =
            (p_b1 * ((variables.b6 * variables.b6) >> 12)) >> 16;
        variables.x3 = ((variables.x1 + variables.x2) + 2) >> 2;
        variables.b4 = (p_ac4 *
                        static_cast<std::uint32_t>(variables.x3 + 32768)) >> 15;
        variables.b7 = (static_cast<std::uint32_t>(p_uncompensated_pressure) -
                        variables.b3) * (50000 >> p_oversampling_setting);
